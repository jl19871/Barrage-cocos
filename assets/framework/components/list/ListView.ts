const { ccclass, property, disallowMultiple, menu, executionOrder, requireComponent, executeInEditMode } = _decorator;
import { Node, Component, Enum, tween, _decorator, EventHandler, Tween, ScrollView, Prefab, Layout, Vec2, Size, NodePool, isValid, instantiate, Vec3, Widget, UITransform, CCFloat, CCBoolean, CCInteger, Mask } from 'cc';
...
export default class ListView extends Component {
    ...
    protected __preload(): void {
        if (!this.node.getComponent(ScrollView)) {
            this.initScrollView();
        }
    }
    ...
    private initScrollView() {
        ...
        transform.setAnchorPoint(new Vec2(0.5, 1));

        const scrollView = this.node.addComponent(ScrollView);
        ...
        viewTransform.setAnchorPoint(transform.anchorPoint);
        viewTransform.setContentSize(transform.contentSize);
        viewNode.parent = this.node;
        scrollView.content = viewNode;

        const viewMask = viewNode.addComponent(Mask);
        viewMask.inverted = false;
        viewMask.type = Mask.Type.GRAPHICS_RECT;

        const viewWidget = viewNode.addComponent(Widget);
        ...
        contentTransform.setAnchorPoint(transform.anchorPoint);
        contentTransform.setContentSize(transform.contentSize);
        ...
        contentLayout.alignVertical = true;
        ...
        contentLayout.affectedByScale = false;

        const contentWidget = contentNode.addComponent(Widget);
